...
User-facing documentation lives in `docs/` and is built with Sphinx + the `furo` theme. The canonical hosted version is <https://upet.readthedocs.io> (Read the Docs, configured via `.readthedocs.yml`); `.github/workflows/docs.yml` additionally deploys a GitHub Pages mirror to `gh-pages` (one directory per tag, plus `latest/` for `main`).
